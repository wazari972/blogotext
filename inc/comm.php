...
function afficher_form_commentaire($article_id, $mode, $erreurs='', $comm_id='') {
	...
	if ($mode == 'admin') {
		...
		$form .= "\t\t".'<button id="button09" class="but" type="button" title="'.$GLOBALS['lang']['bouton-lien'].'" onclick="insertTag(\'[\',\'|]\',\'commentaire'.$rand.'\');"><span></span></button>'."\n";
	...
	// COMMENT ON PUBLIC SIDE
		...
		// ALLOW COMMENTS : OFF
			...
			$form .= "\t\t<br/>".'<label>'.$GLOBALS['lang']['label_dp_captcha'].'<b>'.en_lettres($GLOBALS['captcha']['x']).'</b> &#x0002B; <b>'.en_lettres($GLOBALS['captcha']['y']).'</b> <input type="number" name="captcha" autocomplete="off" value="" class="text" /></label>'."\n";
