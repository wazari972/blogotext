...
function afficher_liste_articles($tableau) {
	if (!empty($tableau)) {
		...
		foreach ($tableau as $article) {
			...
			// TITRE
                        $MD_DEFAULT = False;
			$out .= "\t\t".'<span><span class="'.( ($article['bt_statut'] == '1') ? 'on' : 'off').'"></span>'.'<a href="ecrire.php?post_id='.$article['bt_id'].($MD_DEFAULT ? "&md" : "").'" title="'.trim(mb_substr(strip_tags($article['bt_abstract']), 0, 249)).'">'.$article['bt_title'].'</a>'.'</span>'."\n";
			// DATE
